...
**The form is two fields and no more: the server URL, plus an optional Advanced settings pane holding an OAuth Client ID and an OAuth Client Secret.** The support article walks the Team/Enterprise and the Pro/Max flow step by step and names no other input — *"Add your connector's remote MCP server URL. / Optionally, click 'Advanced settings' to specify an OAuth Client ID and OAuth Client Secret for your server"*, stated once per plan tier — so **there is no custom-HTTP-header field**. Say that as exhaustive enumeration, not as a quotable prohibition: strong, not absolute. **A no-auth server is accepted.** The help article never says so — it only hedges, *"you'll typically go through an OAuth authentication process"* — but the Connectors Directory submission portal settles it, because its Authentication step offers exactly three modes: OAuth, *"a custom connection where users supply their own URL or credentials at connection time"*, or **no authentication** ([submission](https://claude.com/docs/connectors/building/submission), read 2026-08-15). A mode the directory will list is a mode the product accepts. The same step notes you can flag a server that *"starts without authentication and individual tools prompt for it on demand"*.

That is about whether it connects, not whether it should: an unauthenticated remote server is reachable by Anthropic's cloud, so treat no-auth as read-only-and-public-data or not at all — see [`../shared/security-checklist.md`](../shared/security-checklist.md).
