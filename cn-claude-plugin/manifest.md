...
**Substitution in `headers` is limited to a fixed set.** For `http`, `sse` and `ws` servers, the fields `url`, `headers` and `headersHelper` substitute `${CLAUDE_PLUGIN_ROOT}`, `${CLAUDE_PLUGIN_DATA}` and `${CLAUDE_PROJECT_DIR}` — paths, not secrets. Those three are the **whole** list for a plugin-bundled server: the plugins reference enumerates them as the variables Claude Code provides, and generic `${VAR}` or `${VAR:-default}` expansion is **not** among them (read 2026-08-15). A plugin cannot pull a bearer out of the environment — use `${user_config.KEY}` or `headersHelper`, both below. Project `.mcp.json` is the different case: it separately supports **environment-variable expansion** — `${VAR}` and `${VAR:-default}` in `command`, `args`, `env`, `url` and `headers`, with `"Authorization": "Bearer ${API_KEY}"` as the documented example. `The two lists live on two pages and only one of them covers plugins: the plugins reference enumerates the three path variables and stops, while generic `${VAR}` expansion is documented on the Claude Code MCP page for a **project** `.mcp.json`. The reference never says plugin servers are excluded, so this is genuinely ambiguous in the docs — but the safe reading is also the correct design. A plugin is meant to be portable across machines, and a bundled server that silently sends the literal text `Bearer ${MCP_TOKEN}` on any machine where that variable is unset is exactly the failure mode you cannot debug from the user's side. **Do not depend on it.** Use `${user_config.KEY}` or `headersHelper`, both documented for plugins, both below.
