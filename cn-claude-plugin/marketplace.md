...
**A plugin carries MCP two different ways, and which one you get depends on where the server runs.** A bundled **local** MCP server — *"Plugins may include local MCP servers that run on your computer with the same permissions as any other program you run"*, which an Enterprise admin may have *"disabled … entirely"* — reaches only surfaces with a local process, so Desktop and Cowork, never claude.ai chat. A bundled **connector**, and a Claude connector *is* a remote MCP server — *"plugins can also bundle connectors, so the right services are set up for a workflow without you connecting each one"* — reaches claude.ai chat too, and inherits every connector constraint: *"In Cowork, connectors reach external services through Anthropic's cloud, not through your local network. A custom connector must point to a server that's reachable over the public internet from Anthropic's IP ranges."* For a **remote HTTPS** server, the only kind this repo builds, the connector path is the one you get; the local-server path is someone else's plugin. **There is no separate key: it is `.mcp.json`, and a connector is just a remote entry in it.** Resolved 2026-08-15 from [Submitting your plugin](https://claude.com/docs/plugins/submit) — *"Plugins can contain any MCP, including remote MCPs, local MCPs, and MCPBs"*, and *"a plugin can include any MCP of any kind in its `.mcp.json` definition"*. So the `type: "http"` entry you would write for Claude Code is the same entry that ships the connector; nothing else declares it. The reason a full sweep of `code.claude.com/docs` found no answer is that the file is documented there for Claude Code and the naming lives on `claude.com/docs`. A plugin is a distribution channel for **both**, not Claude Code alone.

Three things that page settles alongside it. **Two directories exist and they are not the same thing** — the plugin directory (surfaced in Claude Code as the `claude-plugins-official` marketplace) is *"separate and complementary"* to the [Connectors Directory](https://claude.com/docs/connectors/directory), which is for MCP connectors alone. **Submission is a public GitHub link**, run `claude plugin validate` first, and *"closed-source plugins are not accepted"*; after publication, pushes to that repo are mirrored automatically with no re-submission. And **an individual can submit**: claude.ai's form needs a Team or Enterprise org, but Console ([platform.claude.com](https://platform.claude.com/plugins/submit)) accepts a Developer, Admin or Owner on a Console org, which is the path for an author with no organization behind them.

One more worth knowing before you bundle: a plugin may ship a **`SETUP.md` skill** that walks Claude through configuring the MCP servers it carries — the supported way to hand a user a connection that needs steps.
