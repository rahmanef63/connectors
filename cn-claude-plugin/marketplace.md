...
Each path in `skills` is relative to `source.path`, may be more than one level deep, and registers as `<plugin-name>:<skill-name>`. Live entry from Anthropic's catalog:
...
`strict:false` and the `skills` array are **independent knobs** — the catalog's `box` entry uses `skills` without `strict:false` because its source does have a manifest. For entries rooted at the marketplace itself, listing specific subdirs makes those the complete set; if none of the listed paths exist, the default scan runs anyway.
...
**Where plugins run now:** claude.ai chat, the Claude Desktop Chat tab and Cowork, via Customize → Plugins → Browse plugins → Install. Skills work in chat and Desktop; hooks and sub-agents run only in Cowork and are greyed out in chat. Plugins do carry MCP on these surfaces: the help article says each plugin *"bundles skills, connectors, and sub-agents into a single package"* and that *"plugins can also bundle connectors, so the right services are set up for a workflow without you connecting each one"* — and a Claude connector **is** a remote MCP server. TODO: verify which mechanism delivers it: a plugin's bundled `.mcp.json`, or a separately-registered claude.ai connector the plugin merely references. This survived a full sweep of `code.claude.com/docs` — the plugins reference documents `.mcp.json` for Claude Code only, and the claude.ai help article names connectors without naming the file. Either way a plugin is a distribution channel for **both** Claude Code and claude.ai, not Claude Code alone.
